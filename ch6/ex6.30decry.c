...
int main(int argc, char *argv[]){
	...
	FILE *inp = fopen(argv[1], "r");	/* file di input in sola lettura */
	FILE *out = fopen(argv[2], "w");	/* output sola sovrascrittura */
	char c;

	while((c = fgetc(inp)) != EOF){		/*legge un carattere alla volta */	
		if(isspace(c))
			fprintf(out, "%c", ' ');
		for(int i = 0; i < 52; ++i){
			if(c == la[i])
